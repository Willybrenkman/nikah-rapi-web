...
export default function ForgotPassword() {
    ...
    const handleSend = async (e) => {
        ...
        if (error) { toast.error('Gagal kirim email. Coba lagi!') }
        else { setSent(true) }
        setLoading(false)
    }
    ...
    if (sent) return (
        <div style={pg}>
            ...
            <div className="animate-fade-up auth-card">
                ...
                <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: 24, color: '#2C1810', marginBottom: 8 }}>Email Terkirim!</h2>
                <p style={{ fontSize: 14, color: '#9B8070', lineHeight: 1.7, marginBottom: 8 }}>Link reset password sudah dikirim ke:</p>
                ...
                <div style={{ background: 'rgba(201,149,108,.08)', border: '1px solid #F0E6DF', borderRadius: 12, padding: '16px 20px', marginBottom: 24, textAlign: 'left' }}>
                    <p style={{ fontSize: 13, color: '#9B8070', lineHeight: 1.8 }}>
                        ...
                        3. Buat password baru kamu 🔑
                    </p>
                </div>
                <button onClick={() => navigate('/login')} style={{ ...btn, width: '100%' }}>Kembali ke Login</button>
        ...
        <div style={pg}>
            ...
            <div className="animate-fade-up auth-card">
                <div style={{ textAlign: 'center', marginBottom: 8 }}>
                    ...
                    <p style={{ fontSize: 13, color: '#9B8070', fontStyle: 'italic', marginTop: 4 }}>Reset password akun kamu 🔑</p>
                </div>
                <div style={{ height: 1, background: '#F0E6DF', margin: '24px 0' }} />
                <p style={{ fontSize: 14, color: '#9B8070', marginBottom: 20, lineHeight: 1.6 }}>
                    Masukkan email yang kamu daftarkan. Kami akan kirim link untuk reset password.
                </p>
                <form onSubmit={handleSend}>
                    <div style={{ marginBottom: 20 }}>
                        <label style={{ display: 'block', fontSize: 13, fontWeight: 500, color: '#2C1810', marginBottom: 6 }}>Email</label>
                        <input type="email" className="form-input" placeholder="email@contoh.com" value={email} onChange={e => setEmail(e.target.value)} />
                    </div>
